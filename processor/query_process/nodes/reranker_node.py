...
class ReRankerNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        #对rrf_chunk和web格式统一
        reranker_inputs = self._collect_rerank_inputs(state)

        #bge-rerank打分，并按分数排序
        rewritten_query = state.get("rewritten_query")
        reranked_docs = self._rerank(rewritten_query,reranker_inputs)

        #断崖监测
        final_docs = self._cliff(reranked_docs)
        # json_str = json.dumps(final_docs, indent=4, ensure_ascii=False)
        # print(json_str)
        state["reranked_docs"] = final_docs

        return state

    def _collect_rerank_inputs(self, state):
        result = []
    ...
    def _rerank(self, rewritten_query, reranker_inputs):
        reranker_docs = []
        reranker_client = AIClients.get_bge_rerank_client()
        qd_pairs = [
            (rewritten_query,input_dict.get("content"))
            for input_dict in reranker_inputs
        ]
        #分数列表
        scores = reranker_client.compute_score(qd_pairs)
        for i, score in enumerate(scores):
            doc = reranker_inputs[i]
            reranker_docs.append({**doc,"score":score})

        #排序
        reranker_docs = sorted(reranker_docs, key=lambda x: x["score"], reverse=True)
    ...
    def _normalize_scores(self,scores):
        ...
        normalize_scores = []
        for i, score in enumerate(scores):
            new_score = (score - min_score) / (max_score - min_score) if max_score != min_score else 0
            normalize_scores.append(new_score)
